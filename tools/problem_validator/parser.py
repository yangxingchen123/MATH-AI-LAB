"""YAML Front Matter extraction and parsing for Problem Validator v1."""
...
def parse_markdown_file(path: Path, project_root: Path) -> ParseResult:
    ...
    result = ParseResult(
        path=path,
        relative_path=rel,
        has_front_matter=raw_yaml is not None or any(i.rule_id == "P-PARSE-E002" for i in extract_issues),
        issues=[],
        raw_yaml=raw_yaml,
        body=body,
    )
    ...
    if raw_yaml is None:
        ...
        result.issues.append(
            ValidationIssue(
                severity=Severity.ERROR,
                rule_id="P-PARSE-E005",
                message="Problem data file must have YAML Front Matter.",
                file=rel,
            )
        )
        return result
