...
先读 `.cursor/skills/math-modeling-lab/SKILL.md`。本仓库目录职责优先于上游「每阶段停下来问一次」。

- 竞赛整卷不进 `02_题目库/`
- Dossier：`07_项目/<name>/`
- 代码：`05_代码/<name>/`；模板副本：`05_代码/_模板/数模竞赛_v1/`
- 论文：`04_LATEX/数学建模/<name>/`
- 本目录 `references/` 就是上游手册、Playbook 与代码模板，直接读这里
- `vendor/math-modeling-skills/` 只作 MIT pin，不要改
- AUTO 默认一轮做完并落盘；STUDY 才按上游分阶段停
- 不得自动创建 Knowledge，未授权不发布 PDF
