...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m tools.method_validator",
        description=(
            "Method Metadata Validator v1 "
            "(validates Frozen Method Schema v1). Read-only; Knowledge dependency-aware."
        ),
    )
    ...
    check = sub.add_parser(
        "check",
        help="Validate Knowledge dependency and all Methods under 12_方法库/.",
    )
    add_common(check)

    check_file = sub.add_parser(
        "check-file",
        help="Validate one file with full Method registry and Knowledge context.",
    )
    check_file.add_argument("path", type=Path, help="Path to a Method Markdown file.")
