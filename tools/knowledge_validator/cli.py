"""CLI for Knowledge Validator v1.1."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m tools.knowledge_validator",
        description=(
            "Knowledge Metadata Validator v1.1 "
            "(validates Frozen Knowledge Schema v1). Read-only; no auto-fix."
        ),
    )
    ...
    check = sub.add_parser("check", help="Validate all Knowledge under 01_知识库/.")
    add_common(check)

    check_file = sub.add_parser(
        "check-file",
        help="Validate one file with full Knowledge registry context.",
    )
    check_file.add_argument("path", type=Path, help="Path to a Knowledge Markdown file.")
...
def run(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "check":
            result = validate_project(root=args.root, verbose=args.verbose)
        elif args.command == "check-file":
            result = validate_file(args.path, root=args.root, verbose=args.verbose)
        else:
    ...
    if args.format == "json":
        # Keep stdout pure JSON; no mixed logs.
