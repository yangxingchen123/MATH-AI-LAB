"""Orchestration: Knowledge dependency → discover → parse → validate → registry."""
...
from .rules_object import validate_base, validate_knowledge, validate_unique_ids
        ...
        issues.append(
            ValidationIssue(
                ...
                message=(
                    ...
                    "Method → Knowledge target validation is unavailable."
                ),
...
def _build_registry(documents: list[MethodDocument]) -> dict[str, MethodDocument]:
    registry: dict[str, MethodDocument] = {}
...
def _build_documents(
    project_root: Path,
    paths: list[Path],
) -> tuple[list[MethodDocument], list[ValidationIssue]]:
    documents: list[MethodDocument] = []
    issues: list[ValidationIssue] = []
...
def validate_project(
    ...
    *,
    ...
    included = discover_markdown_files(project_root)
    ...
    result = ValidationResult(
        ...
        summary=ValidationSummary(markdown_files=len(included)),
        documents=documents,
    ...
    result.summary.method_objects = len(documents)
    ...
    *,
        ...
        if included is not None and focus not in included_resolved:
            result.add_issue(
                ValidationIssue(
                    severity=Severity.ERROR,
                    rule_id="M-DISC-E002",
                    message=f"check-file target is outside Method library: {rel}",
                    file=rel,
                )
            )
    ...
    for issue in result.issues:
        if issue.file == rel:
            ...
            continue
        if issue.rule_id == "M-ID-E001" and issue.details:
            files = issue.details.get("files") or []
            if rel in files:
                filtered.append(issue)
                continue
        if issue.rule_id == "M-KNOW-E010":
            filtered.append(issue)
