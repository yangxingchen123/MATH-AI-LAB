"""Text and JSON reporting for Problem Validator v1."""
...
def result_to_dict(result: ValidationResult) -> dict[str, Any]:
    return {
        "validator": f"problem_validator v{VALIDATOR_VERSION}",
...
def format_text(result: ValidationResult, *, summary_only: bool = False, verbose: bool = False) -> str:
    lines: list[str] = []
    lines.append(f"Problem Validator v{VALIDATOR_VERSION}")
    lines.append(f"Schema: Problem v{SCHEMA_VERSION} ({SCHEMA_STATUS.capitalize()}, {SCHEMA_FROZEN_DATE})")
    lines.append("")
    ...
    if summary_only:
        lines.append(f"Problem objects: {result.summary.problem_objects}")
