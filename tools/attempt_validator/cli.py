...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m tools.attempt_validator",
        description=(
            "Attempt Metadata Validator v1 "
            "(validates Frozen Attempt Schema v1). Read-only; Problem dependency-aware."
        ),
    )
    ...
    check = sub.add_parser(
        "check",
        help="Validate Problem dependency and all Attempts under 11_学习证据/尝试记录/.",
    )
    add_common(check)

    check_file = sub.add_parser(
        "check-file",
        help="Validate one file with full Attempt registry and Problem context.",
    )
    check_file.add_argument("path", type=Path, help="Path to an Attempt Markdown file.")
